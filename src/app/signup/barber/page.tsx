...
export default function SignUpBarberPage() {
  ...
  const handleSignup = async (data: SignUpFormValues) => {
    setError(null);
    try {
      await signUpBarberWithEmail(data.email, data.password, data.fullName, data.phone);
      setStep('verify');
    } catch (err: any) {
       ...
       <main className="flex min-h-screen w-full items-center justify-center bg-background p-4 font-body">
         <Card className="w-full max-w-sm text-center border-none shadow-lg">
            ...
            <CardContent>
                ...
                <AlertDescription>
                    Enviamos um link de verificação para <strong>{form.getValues('email')}</strong>. Por favor, clique no link para ativar sua conta de barbeiro.
                </AlertDescription>
