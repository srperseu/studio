    ...
    <>
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="bg-gray-800 border-gray-700">
          ...
          <div className="text-gray-300 bg-gray-700 p-4 rounded-md whitespace-pre-wrap">{modalContent.body}</div>
          <DialogFooter>
      ...
      <header className="flex flex-col sm:flex-row justify-between items-center mb-8">
        ...
        <div className="flex items-center gap-4 mt-4 sm:mt-0">
          <span className="text-gray-300 hidden md:block">Olá, {barberData.fullName}</span>
              ...
              {appointments.length > 0 ? (
                <div className="space-y-4">
                  {appointments.map(app => (
                    <div key={app.id} className="bg-gray-700/70 p-4 rounded-lg flex flex-col sm:flex-row justify-between items-center gap-4">
                      <div className="flex-grow">
                        <p className="font-bold text-lg text-primary">{app.clientName}</p>
                        <p className="text-gray-300">{app.service}</p>
                      ...
                      <div className="flex flex-col sm:items-end gap-2">
                         <Badge className={`${app.type === 'inShop' ? 'bg-blue-500' : 'bg-green-500'} text-white`}>{app.type === 'inShop' ? 'Na Barbearia' : 'Em Domicílio'}</Badge>
          ...
          <Card className="bg-gray-800/80 border-gray-700 shadow-lg">
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <p>{barberData.address || 'Não informado'}</p>
              </div>
              <div>
                <h3 className="font-bold text-primary">Serviços</h3>
                {barberData.services?.inShop?.active && <p>Na Barbearia: R$ {barberData.services.inShop.price}</p>}
                {barberData.services?.atHome?.active && <p>Em Domicílio: R$ {barberData.services.atHome.price}</p>}
              </div>
              <div>
